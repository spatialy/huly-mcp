...
export const getProject = (
  ...
  Effect.gen(function*() {
    const { defaultStatusRef, project, statuses } = yield* findProjectWithStatuses(params.project)

    const defaultStatus = defaultStatusRef !== undefined
      ? statuses.find((s) => s._id === defaultStatusRef)
      : statuses[0]
